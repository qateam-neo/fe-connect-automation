...
from pages.investment_proposal.customized_plan.config import ELEMENTS, CustomizedFlowData


class CustomizedFlow(BasePage):
    ...
    def answer_bigger_investment_risks_question(self):
        bigger_investment_risks_combo_locator = ELEMENTS.BIGGER_INVESTMENT_RISKS_COMBO
        self.wait_scroll_vertically_to_element_by_xpath_and_fill(bigger_investment_risks_combo_locator)
        bigger_investment_risks_item_locator = ELEMENTS.BIGGER_INVESTMENT_RISKS_ITEM
    ...
    def answer_worry_about_losing_money_question(self):
        worry_about_losing_money_combo_locator = ELEMENTS.WORRY_ABOUT_LOSING_MONEY_COMBO
        self.wait_and_click_element_by_xpath(worry_about_losing_money_combo_locator)
        worry_about_losing_money_item_locator = ELEMENTS.WORRY_ABOUT_LOSING_MONEY_ITEM
        self.wait_and_click_element_by_xpath(worry_about_losing_money_item_locator)
        

 # def choose_monthly_income(self):
        # wait = WebDriverWait(self.driver, 50)
        # monthly_income_combo = wait.until(EC.visibility_of_element_located((AppiumBy.XPATH, ELEMENTS.MONTHLY_INCOME_COMBO)))
        ...
        #     WebDriverWait(self.driver, 50).until(EC.presence_of_element_located((AppiumBy.XPATH, ELEMENTS.MONTHLY_INCOME_ITEM_ID))).click()
